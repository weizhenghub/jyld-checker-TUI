...
🔑 凭据智能解析：只需要sess或api-key便可查询（apikey已不支持，推荐sess）  
📊 三接口查询：账号信息（/auth/me）+ 钱包余额（/wallet/summary）+ 用量统计（/usage-summary），并行拉取、一次展示  
📈 TUI 仪表盘：每个 sess 一个容器，默认显示 用户名 / 手机号 / 状态 / 可用余额 / 赠送余额，点「高级状态」展开全部字段  
